...
// ── Field sorti du composant pour éviter le remontage à chaque frappe ──
interface FieldProps {
  ...
  textarea?: boolean
  editing: boolean
  form: Partial<Structure>
  structure: Structure
  onChange: (field: keyof Structure, value: string) => void
}

function Field({ label, field, type = 'text', textarea = false, editing, form, structure, onChange }: FieldProps) {
  return (
    <div className="mb-3">
...
export default function FicheModal({ structure, scriptBase, onClose, onUpdate }: Props) {
  ...
  const handleFieldChange = useCallback((field: keyof Structure, value: string) => {
    setForm(prev => ({ ...prev, [field]: value }))
  }, [])

  const fieldProps = { editing, form, structure, onChange: handleFieldChange }
        ...
        <div className="sticky top-0 z-10 bg-ardoise-900 border-b border-ardoise-700 px-5 py-4">
          ...
          <div className="mt-3">
            {editing ? (
              <select
                value={form.statut}
                onChange={e => setForm(prev => ({ ...prev, statut: e.target.value as Statut }))}
          ...
          <div className="mt-4 flex gap-0 border-b border-ardoise-700 -mb-4">
            <button className="px-4 py-2 text-xs font-mono uppercase tracking-widest border-b-2 border-amber-500 text-amber-400 transition-colors">
              Fiche
            </button>
            <button
            ...
            >
              Score
              {liveNiv && (
                <span className="w-1.5 h-1.5 rounded-full inline-block" style={{ backgroundColor: liveMeta?.couleur }} />
              )}
        ...
        <div className="flex-1 overflow-y-auto px-5 py-4">
          ...
          <div className="mb-5">
            <div className="text-xs font-mono text-slate-600 uppercase tracking-widest mb-3 flex items-center gap-2">
              <span className="w-4 h-px bg-ardoise-600 inline-block" />
              Localisation
            </div>
            <Field label="Adresse" field="adresse" {...fieldProps} />
            <div className="grid grid-cols-2 gap-3">
              <Field label="Ville" field="ville" {...fieldProps} />
              <Field label="Code postal" field="code_postal" {...fieldProps} />
            </div>
          ...
          <div className="mb-5">
            ...
            <Field label="Nom" field="contact_nom" {...fieldProps} />
            <Field label="Téléphone" field="contact_tel" type="tel" {...fieldProps} />
            <Field label="Email" field="contact_email" type="email" {...fieldProps} />
          </div>
          ...
          <div className="mb-5">
            ...
            <Field label="" field="notes" textarea {...fieldProps} />
          </div>
